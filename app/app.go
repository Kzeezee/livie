...
func New(cfg *config.Config, mgr *runner.Manager, agt *agent.Agent, indexer *index.Indexer, store *index.Store) Model {
	w, h := 120, 36
	...
	return Model{
		...
		chat:    screens.NewChatModel(cfg, mgr, agt, indexer, store, w, h),
		cfg:     cfg,
		runner:  mgr,
		agent:   agt,
		indexer: indexer,
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if tt, ok := msg.(screens.TransitionToChat); ok {
		...
		m.chat = screens.NewChatModel(m.cfg, m.runner, m.agent, m.indexer, m.store, m.width, m.height)
